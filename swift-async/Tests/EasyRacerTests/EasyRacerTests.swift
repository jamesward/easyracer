...
final class EasyRacerTests: XCTestCase {
    func testAllScenarios() async {
        ...
        do {
            ...
            // Test
            let instance = EasyRacer(baseURL: baseURL)
            XCTAssertEqual(instance.scenarios.count, 11, "Number of Scenarios")
            for (num, scenario) in instance.scenarios {
                let result = await scenario()
                XCTAssertEqual(result, "right", "Scenario \(num)")
            }
