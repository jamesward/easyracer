...
type ResourceFactory(resource: IObservable<String>, close: IObservable<String>) =
  ...
  interface IDisposable with
    member this.Dispose() = close |> Async.AwaitObservable |> Async.RunSynchronously |> ignore
...
let scenario8 (scenarioGet: string -> IObservable<HttpResponseMessage>) : IObservable<string> =
    ...
    let reqRes =
      Observable.usingAsync
        (fun _ -> reqOpen |> Observable.map (fun id -> new ResourceFactory(reqUse id, reqClose id)) |> Async.AwaitObservable)
